...
- **risk-shield's scheduler downloads live on a cadence** (Part 3.4). It runs only where `SCHEDULER_ENABLED=true` (the prod compose service).
  ...
  - **Loops wait only through `wallclock.sleep_until`** (3.4 follow-up). A single `asyncio.sleep` or `wait_for` toward a far target runs on Docker's monotonic clock, which stops while the Mac sleeps (decisions 2026-09-11). The health payload's `PAYLOAD_KEYS` is append-only.
...
- **The macro brief stays off until ai-agent's `POST /brief/macro` exists** (Part 3.6a; 3.6b generates).
